...
const AdminPage = () => {
  ...
  const fetchUsers = async () => {
    const response = await axios.get('http://localhost:8080/login/pendingUsers', {
  ...
  const approveUser = async (userId) => {
    try {
      await axios.put(`http://localhost:8080/login/pendingUsers/${userId}`, {
        access: 1
      }, { withCredentials: true });
  ...
  const rejectUser = async (userId) => {
    try {
      await axios.delete(`http://localhost:8080/login/pendingUsers/${userId}`, {
        withCredentials : true
      });
